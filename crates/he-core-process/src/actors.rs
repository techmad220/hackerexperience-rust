...
use he_core_core::{CoreError, ProcessId as CoreProcessId};
...
impl Message for CreateProcess {
    type Result = Result<Process, CoreError>;
}
...
impl Message for GetProcess {
    type Result = Result<Option<Process>, CoreError>;
}
...
impl Message for GetServerProcesses {
    type Result = Result<Vec<Process>, CoreError>;
}
...
impl Message for DeallocateResources {
    type Result = Result<ProcessResources, CoreError>;
}
...
impl Handler<CreateProcess> for ProcessActor {
    async fn handle(&mut self, msg: CreateProcess, _ctx: &mut ActorContext) -> Result<Process, CoreError> {
...
impl Handler<GetProcess> for ProcessActor {
    async fn handle(&mut self, msg: GetProcess, _ctx: &mut ActorContext) -> Result<Option<Process>, CoreError> {
...
impl Handler<UpdateProcess> for ProcessActor {
    async fn handle(&mut self, msg: UpdateProcess, _ctx: &mut ActorContext) -> Result<Process, CoreError> {
        let mut processes = self.processes.write().await;
        
        let process = processes.get_mut(&msg.process_id)
            .ok_or_else(|| CoreError::not_found("Process not found"))?;
...
impl Handler<StartProcess> for ProcessActor {
    async fn handle(&mut self, msg: StartProcess, _ctx: &mut ActorContext) -> Result<(), CoreError> {
        let mut processes = self.processes.write().await;
        ...
        if !matches!(process.state, ProcessState::Waiting | ProcessState::Paused) {
            return Err(CoreError::validation("Process cannot be started in current state"));
        }
...
impl Handler<PauseProcess> for ProcessActor {
    async fn handle(&mut self, msg: PauseProcess, _ctx: &mut ActorContext) -> Result<(), CoreError> {
        let mut processes = self.processes.write().await;
        let mut contexts = self.execution_contexts.write().await;
        ...
        if !matches!(process.state, ProcessState::Running) {
            return Err(CoreError::validation("Process is not running"));
        }
...
impl Handler<ResumeProcess> for ProcessActor {
    async fn handle(&mut self, msg: ResumeProcess, _ctx: &mut ActorContext) -> Result<(), CoreError> {
        let mut processes = self.processes.write().await;
        let mut contexts = self.execution_contexts.write().await;
        ...
        if !matches!(process.state, ProcessState::Paused) {
            return Err(CoreError::validation("Process is not paused"));
        }
...
impl Handler<KillProcess> for ProcessActor {
    async fn handle(&mut self, msg: KillProcess, _ctx: &mut ActorContext) -> Result<(), CoreError> {
        let mut processes = self.processes.write().await;
        let mut contexts = self.execution_contexts.write().await;
        ...
        if let Some(process) = processes.get_mut(&msg.process_id) {
            ...
            Err(CoreError::not_found("Process not found"))
        }
    }
}
...
impl Handler<DeleteProcess> for ProcessActor {
    async fn handle(&mut self, msg: DeleteProcess, _ctx: &mut ActorContext) -> Result<(), CoreError> {
        let mut processes = self.processes.write().await;
        let mut contexts = self.execution_contexts.write().await;
        let mut resource_allocations = self.resource_allocations.write().await;
...
impl Handler<GetServerProcesses> for ProcessActor {
    async fn handle(&mut self, msg: GetServerProcesses, _ctx: &mut ActorContext) -> Result<Vec<Process>, CoreError> {
        let processes = self.processes.read().await;
...
impl Handler<GetProcessesByType> for ProcessActor {
    async fn handle(&mut self, msg: GetProcessesByType, _ctx: &mut ActorContext) -> Result<Vec<Process>, CoreError> {
        let processes = self.processes.read().await;
...
impl Handler<AllocateResources> for ProcessActor {
    async fn handle(&mut self, msg: AllocateResources, _ctx: &mut ActorContext) -> Result<(), CoreError> {
        let mut resource_allocations = self.resource_allocations.write().await;
        let processes = self.processes.read().await;
        
        // Verify process exists
        if !processes.contains_key(&msg.process_id) {
            return Err(CoreError::not_found("Process not found"));
        }
...
impl Handler<DeallocateResources> for ProcessActor {
    async fn handle(&mut self, msg: DeallocateResources, _ctx: &mut ActorContext) -> Result<ProcessResources, CoreError> {
        let mut resource_allocations = self.resource_allocations.write().await;
        ...
        if let Some(resources) = resource_allocations.remove(&msg.process_id) {
            ...
            Err(CoreError::not_found("Process resource allocation not found"))
        }
    }
}
...
pub struct ProcessSupervisor {
    process_actor: Option<he_core_core::actors::ActorAddress>,
}
...
impl ProcessSupervisor {
    ...
    pub fn get_process_actor(&self) -> Option<&he_core_core::actors::ActorAddress> {
