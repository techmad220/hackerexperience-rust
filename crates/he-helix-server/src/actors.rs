...
use he_helix_core::{HelixError, ProcessId};
...
impl Message for CreateServer {
    type Result = Result<Server, HelixError>;
}
...
impl Message for GetServer {
    type Result = Result<Option<Server>, HelixError>;
}
...
impl Message for DetachComponent {
    type Result = Result<Component, HelixError>;
}
...
impl Message for GetServerComponents {
    type Result = Result<Vec<Component>, HelixError>;
}
...
impl Message for GetServerResources {
    type Result = Result<Resources, HelixError>;
}
...
impl Handler<CreateServer> for ServerActor {
    async fn handle(&mut self, msg: CreateServer, _ctx: &mut ActorContext) -> Result<Server, HelixError> {
...
impl Handler<GetServer> for ServerActor {
    async fn handle(&mut self, msg: GetServer, _ctx: &mut ActorContext) -> Result<Option<Server>, HelixError> {
...
impl Handler<UpdateServer> for ServerActor {
    async fn handle(&mut self, msg: UpdateServer, _ctx: &mut ActorContext) -> Result<Server, HelixError> {
        let mut servers = self.servers.write().await;
        
        let server = servers.get_mut(&msg.server_id)
            .ok_or_else(|| HelixError::not_found("Server not found"))?;
...
impl Handler<DeleteServer> for ServerActor {
    async fn handle(&mut self, msg: DeleteServer, _ctx: &mut ActorContext) -> Result<(), HelixError> {
        let mut servers = self.servers.write().await;
        let mut states = self.states.write().await;
        let mut components = self.components.write().await;
        ...
        if servers.remove(&msg.server_id).is_some() {
            ...
            Err(HelixError::not_found("Server not found"))
        }
    }
}
...
impl Handler<AttachComponent> for ServerActor {
    async fn handle(&mut self, msg: AttachComponent, _ctx: &mut ActorContext) -> Result<(), HelixError> {
        ...
        if !self.can_attach_component(&msg.server_id, &msg.component).await {
            return Err(HelixError::validation("Component cannot be attached to this server"));
        }
...
impl Handler<DetachComponent> for ServerActor {
    async fn handle(&mut self, msg: DetachComponent, _ctx: &mut ActorContext) -> Result<Component, HelixError> {
        let mut components = self.components.write().await;
        let mut servers = self.servers.write().await;
        let mut motherboards = self.motherboards.write().await;
        ...
        let component_index = server_components
            ...
            .ok_or_else(|| HelixError::not_found("Component not found"))?;
...
impl Handler<GetServerComponents> for ServerActor {
    async fn handle(&mut self, msg: GetServerComponents, _ctx: &mut ActorContext) -> Result<Vec<Component>, HelixError> {
        let components = self.components.read().await;
...
impl Handler<GetServerResources> for ServerActor {
    async fn handle(&mut self, msg: GetServerResources, _ctx: &mut ActorContext) -> Result<Resources, HelixError> {
...
impl Handler<StartServer> for ServerActor {
    async fn handle(&mut self, msg: StartServer, _ctx: &mut ActorContext) -> Result<(), HelixError> {
        let mut states = self.states.write().await;
        let servers = self.servers.read().await;
        
        if !servers.contains_key(&msg.server_id) {
            return Err(HelixError::not_found("Server not found"));
        }
        ...
        match current_state {
            ...
            _ => Err(HelixError::validation("Server cannot be started in current state"))
        }
    }
}
...
impl Handler<StopServer> for ServerActor {
    async fn handle(&mut self, msg: StopServer, _ctx: &mut ActorContext) -> Result<(), HelixError> {
        let mut states = self.states.write().await;
        let servers = self.servers.read().await;
        ...
        match current_state {
            ...
            _ => Err(HelixError::validation("Server cannot be stopped in current state"))
        }
    }
}
...
impl Handler<RebootServer> for ServerActor {
    async fn handle(&mut self, msg: RebootServer, _ctx: &mut ActorContext) -> Result<(), HelixError> {
        let mut states = self.states.write().await;
        let servers = self.servers.read().await;
...
pub struct ServerSupervisor {
    server_actor: Option<he_helix_core::actors::ActorAddress>,
}
...
impl ServerSupervisor {
    ...
    pub fn get_server_actor(&self) -> Option<&he_helix_core::actors::ActorAddress> {
