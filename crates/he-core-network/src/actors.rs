...
impl Message for CreateConnection {
    type Result = Result<Connection, CoreError>;
}
...
impl Message for GetConnection {
    type Result = Result<Option<Connection>, CoreError>;
}
...
impl Message for GetServerConnections {
    type Result = Result<Vec<Connection>, CoreError>;
}
...
impl Message for CreateTunnel {
    type Result = Result<Tunnel, CoreError>;
}
...
impl Message for GetTunnel {
    type Result = Result<Option<Tunnel>, CoreError>;
}
...
impl Message for CreateNetwork {
    type Result = Result<Network, CoreError>;
}
...
impl Message for GetNetworkMembers {
    type Result = Result<Vec<ServerId>, CoreError>;
}
...
impl NetworkActor {
    ...
    fn validate_connection(&self, source: &ServerId, target: &ServerId) -> Result<(), CoreError> {
        if source == target {
            return Err(CoreError::validation("Cannot connect server to itself"));
        }
        Ok(())
    }
...
impl Handler<CreateConnection> for NetworkActor {
    async fn handle(&mut self, msg: CreateConnection, _ctx: &mut ActorContext) -> Result<Connection, CoreError> {
...
impl Handler<GetConnection> for NetworkActor {
    async fn handle(&mut self, msg: GetConnection, _ctx: &mut ActorContext) -> Result<Option<Connection>, CoreError> {
...
impl Handler<CloseConnection> for NetworkActor {
    async fn handle(&mut self, msg: CloseConnection, _ctx: &mut ActorContext) -> Result<(), CoreError> {
        let mut connections = self.connections.write().await;
        ...
        if let Some(connection) = connections.get_mut(&msg.connection_id) {
            ...
            Err(CoreError::not_found("Connection not found"))
        }
    }
}
...
impl Handler<GetServerConnections> for NetworkActor {
    async fn handle(&mut self, msg: GetServerConnections, _ctx: &mut ActorContext) -> Result<Vec<Connection>, CoreError> {
        let connections = self.connections.read().await;
...
impl Handler<CreateTunnel> for NetworkActor {
    async fn handle(&mut self, msg: CreateTunnel, _ctx: &mut ActorContext) -> Result<Tunnel, CoreError> {
...
impl Handler<GetTunnel> for NetworkActor {
    async fn handle(&mut self, msg: GetTunnel, _ctx: &mut ActorContext) -> Result<Option<Tunnel>, CoreError> {
...
impl Handler<CloseTunnel> for NetworkActor {
    async fn handle(&mut self, msg: CloseTunnel, _ctx: &mut ActorContext) -> Result<(), CoreError> {
        let mut tunnels = self.tunnels.write().await;
        ...
        if let Some(tunnel) = tunnels.get_mut(&msg.tunnel_id) {
            ...
            Err(CoreError::not_found("Tunnel not found"))
        }
    }
}
...
impl Handler<CreateNetwork> for NetworkActor {
    async fn handle(&mut self, msg: CreateNetwork, _ctx: &mut ActorContext) -> Result<Network, CoreError> {
...
impl Handler<JoinNetwork> for NetworkActor {
    async fn handle(&mut self, msg: JoinNetwork, _ctx: &mut ActorContext) -> Result<(), CoreError> {
        let mut networks = self.networks.write().await;
        let mut memberships = self.network_memberships.write().await;

        let network = networks.get_mut(&msg.network_id)
            .ok_or_else(|| CoreError::not_found("Network not found"))?;
...
impl Handler<LeaveNetwork> for NetworkActor {
    async fn handle(&mut self, msg: LeaveNetwork, _ctx: &mut ActorContext) -> Result<(), CoreError> {
        let mut networks = self.networks.write().await;
        let mut memberships = self.network_memberships.write().await;
...
impl Handler<GetNetworkMembers> for NetworkActor {
    async fn handle(&mut self, msg: GetNetworkMembers, _ctx: &mut ActorContext) -> Result<Vec<ServerId>, CoreError> {
...
impl Handler<UpdateConnectionState> for NetworkActor {
    async fn handle(&mut self, msg: UpdateConnectionState, _ctx: &mut ActorContext) -> Result<(), CoreError> {
        let mut connections = self.connections.write().await;
...
pub struct NetworkSupervisor {
    network_actor: Option<he_core_core::actors::ActorAddress>,
}
...
impl NetworkSupervisor {
    ...
    pub fn get_network_actor(&self) -> Option<&he_core_core::actors::ActorAddress> {
