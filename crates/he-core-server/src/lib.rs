...
pub async fn init() -> Result<()> {
    tracing::info!("Initializing Core Server subsystem");
    
    // Initialize the server registry
    let _registry = SERVER_REGISTRY.clone();
    
    tracing::info!("Core Server subsystem initialized successfully");
    Ok(())
}

/// Shutdown the server subsystem gracefully
pub async fn shutdown() -> Result<()> {
    tracing::info!("Shutting down Core Server subsystem");
    
    // Clear the server registry
    let registry = SERVER_REGISTRY.read().await;
    registry.servers.clear();
    
    tracing::info!("Core Server subsystem shutdown complete");
    Ok(())
}