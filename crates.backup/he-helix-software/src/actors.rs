...
impl Message for CreateFile {
    type Result = Result<File, HelixError>;
}
...
impl Message for GetFile {
    type Result = Result<Option<File>, HelixError>;
}
...
impl Message for GetServerFiles {
    type Result = Result<Vec<File>, HelixError>;
}
...
impl Message for CreateSoftware {
    type Result = Result<Software, HelixError>;
}
...
impl Message for GetSoftware {
    type Result = Result<Option<Software>, HelixError>;
}
...
impl Message for GetServerSoftware {
    type Result = Result<Vec<Software>, HelixError>;
}
...
impl Message for CreateVirus {
    type Result = Result<Virus, HelixError>;
}
...
impl Message for GetVirus {
    type Result = Result<Option<Virus>, HelixError>;
}
...
impl Message for ScanForViruses {
    type Result = Result<Vec<Virus>, HelixError>;
}
...
impl SoftwareActor {
    ...
    /// Validate file path
    fn validate_path(&self, path: &str) -> Result<(), HelixError> {
        if path.is_empty() || path.starts_with("..") || path.contains("//") {
            return Err(HelixError::validation("Invalid file path"));
        }
        Ok(())
    }
...
impl Handler<CreateFile> for SoftwareActor {
    async fn handle(&mut self, msg: CreateFile, _ctx: &mut ActorContext) -> Result<File, HelixError> {
...
impl Handler<GetFile> for SoftwareActor {
    async fn handle(&mut self, msg: GetFile, _ctx: &mut ActorContext) -> Result<Option<File>, HelixError> {
...
impl Handler<UpdateFile> for SoftwareActor {
    async fn handle(&mut self, msg: UpdateFile, _ctx: &mut ActorContext) -> Result<File, HelixError> {
        let mut files = self.files.write().await;
        
        let file = files.get_mut(&msg.file_id)
            .ok_or_else(|| HelixError::not_found("File not found"))?;
...
impl Handler<DeleteFile> for SoftwareActor {
    async fn handle(&mut self, msg: DeleteFile, _ctx: &mut ActorContext) -> Result<(), HelixError> {
        let mut files = self.files.write().await;
        ...
        if let Some(file) = files.remove(&msg.file_id) {
            ...
            Err(HelixError::not_found("File not found"))
        }
    }
}
...
impl Handler<GetServerFiles> for SoftwareActor {
    async fn handle(&mut self, msg: GetServerFiles, _ctx: &mut ActorContext) -> Result<Vec<File>, HelixError> {
        let files = self.files.read().await;
...
impl Handler<CreateSoftware> for SoftwareActor {
    async fn handle(&mut self, msg: CreateSoftware, _ctx: &mut ActorContext) -> Result<Software, HelixError> {
...
impl Handler<GetSoftware> for SoftwareActor {
    async fn handle(&mut self, msg: GetSoftware, _ctx: &mut ActorContext) -> Result<Option<Software>, HelixError> {
...
impl Handler<InstallSoftware> for SoftwareActor {
    async fn handle(&mut self, msg: InstallSoftware, _ctx: &mut ActorContext) -> Result<(), HelixError> {
        let mut software_storage = self.software.write().await;
        
        let software = software_storage.get_mut(&msg.software_id)
            .ok_or_else(|| HelixError::not_found("Software not found"))?;

        if software.is_installed {
            return Err(HelixError::validation("Software is already installed"));
        }
...
impl Handler<UninstallSoftware> for SoftwareActor {
    async fn handle(&mut self, msg: UninstallSoftware, _ctx: &mut ActorContext) -> Result<(), HelixError> {
        let mut software_storage = self.software.write().await;
        ...
        if software.server_id != msg.server_id {
            return Err(HelixError::validation("Software is not installed on this server"));
        }
...
impl Handler<GetServerSoftware> for SoftwareActor {
    async fn handle(&mut self, msg: GetServerSoftware, _ctx: &mut ActorContext) -> Result<Vec<Software>, HelixError> {
...
impl Handler<CreateVirus> for SoftwareActor {
    async fn handle(&mut self, msg: CreateVirus, _ctx: &mut ActorContext) -> Result<Virus, HelixError> {
...
impl Handler<GetVirus> for SoftwareActor {
    async fn handle(&mut self, msg: GetVirus, _ctx: &mut ActorContext) -> Result<Option<Virus>, HelixError> {
...
impl Handler<InfectServer> for SoftwareActor {
    async fn handle(&mut self, msg: InfectServer, _ctx: &mut ActorContext) -> Result<(), HelixError> {
        let mut virus_storage = self.viruses.write().await;
        
        let virus = virus_storage.get_mut(&msg.virus_id)
            .ok_or_else(|| HelixError::not_found("Virus not found"))?;
...
impl Handler<ScanForViruses> for SoftwareActor {
    async fn handle(&mut self, msg: ScanForViruses, _ctx: &mut ActorContext) -> Result<Vec<Virus>, HelixError> {
...
impl Handler<RemoveVirus> for SoftwareActor {
    async fn handle(&mut self, msg: RemoveVirus, _ctx: &mut ActorContext) -> Result<(), HelixError> {
        let mut virus_storage = self.viruses.write().await;
        ...
        if let Some(virus) = virus_storage.get_mut(&msg.virus_id) {
            ...
            Err(HelixError::not_found("Virus not found"))
        }
    }
}
...
impl Handler<CopyFile> for SoftwareActor {
    async fn handle(&mut self, msg: CopyFile, _ctx: &mut ActorContext) -> Result<File, HelixError> {
        let files = self.files.read().await;
        
        let source_file = files.get(&msg.source_file_id)
            .ok_or_else(|| HelixError::not_found("Source file not found"))?;
...
impl Handler<MoveFile> for SoftwareActor {
    async fn handle(&mut self, msg: MoveFile, _ctx: &mut ActorContext) -> Result<File, HelixError> {
...
impl Handler<GetFilesByType> for SoftwareActor {
    async fn handle(&mut self, msg: GetFilesByType, _ctx: &mut ActorContext) -> Result<Vec<File>, HelixError> {
        let files = self.files.read().await;
        let server_files = self.server_files.read().await;
...
pub struct SoftwareSupervisor {
    software_actor: Option<he_helix_core::actors::ActorAddress>,
}
...
impl SoftwareSupervisor {
    ...
    pub fn get_software_actor(&self) -> Option<&he_helix_core::actors::ActorAddress> {
